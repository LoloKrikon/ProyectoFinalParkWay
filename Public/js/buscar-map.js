...
import { collection, onSnapshot, query, where, limit, doc, updateDoc } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";

// Inicializa el mapa
const map = L.map('map').setView([40.4168, -3.7038], 15);

// Capa de CartoDB Dark Matter para compatibilidad con el tema oscuro
// Capa de CartoDB Dark Matter (Gratuita y sin API Key restrictiva)
...
let carMarker = null;
// let hasAddedOriginSpots = false; // Código muerto: no se usa realmente o se puede simplificar
...
// Icono de plaza de aparcamiento
    ...
    onSnapshot(q, (snapshot) => {
        snapshot.docChanges().forEach((change) => {
                    ...
                    const marker = L.marker([location.latitude, location.longitude], { icon: parkingIcon })
                        ...
                        .bindPopup(`
                            ...
                            <button onclick="navegarA('${location.latitude}', '${location.longitude}')" 
                                    style="margin-top: 8px; width: 100%; background: #7c3aed; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; margin-bottom: 5px;">
                                    <i class="fa-solid fa-location-arrow"></i> Ir aquí
                                </button>
                                <button onclick="ocuparPlaza('${docId}', ${location.latitude}, ${location.longitude})" 
                                    style="width: 100%; background: #10b981; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer;">
                                    <i class="fa-solid fa-square-check"></i> He aparcado aquí
                                </button>
                            </div>
                        `);
            ...
            if (change.type === "modified") {
                // Si cambia de estado (ej: alguien lo ocupó), lo quitamos del mapa
                if (data.status !== 'free' && markers[docId]) {
...
// Función para Ocupar (Eliminar) plaza
window.ocuparPlaza = async function (docId, lat, lng) {
    if (!carMarker && !lastKnownPos) {
        alert("No podemos verificar tu ubicación. Asegúrate de tener el GPS activo.");
        return;
    }

    // Usamos la posición del coche (virtual o real)
    const userPos = carMarker ? carMarker.getLatLng() : lastKnownPos;
    const spotPos = L.latLng(lat, lng);

    // Calcular distancia en metros
    const dist = userPos.distanceTo(spotPos); // Leaflet method

    // Distancia máxima permitida (Aumentada para facilitar pruebas)
    const MAX_DIST = 150;



    if (dist > MAX_DIST) {
        alert(`❌ Estás demasiado lejos (${dist.toFixed(0)}m).\nDebes acercarte a menos de ${MAX_DIST}m (usa el botón 'Moverme').`);
        return;
    }

    if (!confirm(`¿Estás en la plaza? (Distancia: ${dist.toFixed(0)}m)\nAl aceptar, la plaza desaparecerá del mapa.`)) return;

    try {
        const plazaRef = doc(db, "plazas_libres", docId);
        await updateDoc(plazaRef, {
            status: 'occupied',
            takenBy: 'user_action',
            takenAt: new Date()
        });
        alert("✅ ¡Plaza ocupada! Gracias por avisar.");
    } catch (e) {
        console.error("Error ocupando plaza:", e);
        alert("Error al actualizar la base de datos: " + e.message);
    }
};
...
// let isManualMode = true; // Código muerto
...
function mostrarIndicaciones(e) {
    ...
    // Restaurar botón
    const btn = document.querySelector('button[onclick="iniciarNavegacion()"]');
    btn.innerHTML = '<i class="fa-solid fa-location-arrow"></i> Navegar';
}
